...
public class CompanyPane extends GridPane {
	...
	public CompanyPane() {
		...
		lblCus.setPadding(new Insets(8, 0, 4, 0));
		...
		Button btnCreateCus = new Button("Create Customer");
		hbxButtons.getChildren().add(btnCreateCus);
		btnCreateCus.setOnAction(event -> this.createCustomer());
	...
	private void createCustomer(){
		Company selectedCompany = lvwCompanies.getSelectionModel().getSelectedItem();
		CustomerWindow dia = new CustomerWindow("Create Customer", selectedCompany);
		dia.showAndWait();

		// Wait for the modal dialog to close

		int index = lvwCompanies.getSelectionModel().getSelectedIndex();
		lvwCompanies.getItems().setAll(Controller.getCompanies());
		lvwCompanies.getSelectionModel().select(index);
	}
