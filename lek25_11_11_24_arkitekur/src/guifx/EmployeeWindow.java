...
public class EmployeeWindow extends Stage {
	...
	private TextField txfName, txfWage, txfYearOfEmployment;
	...
	private void initContent(GridPane pane) {
		...
		Label employment = new Label("Year of Employment");
		pane.add(employment, 0, 4);

		txfYearOfEmployment = new TextField();
		pane.add(txfYearOfEmployment, 0, 5);
	...
	private void initControls() {
		if (employee != null) {
			...
			if (employee.getCompany() != null) {
				...
				txfYearOfEmployment.setText("" + employee.getYearOfEmployment());
			} else {
				cbbCompany.getSelectionModel().select(0);
				txfYearOfEmployment.clear();
			}
		} else {
			txfName.clear();
			txfWage.clear();
	...
	private void okAction() {
		...
		if (name.isEmpty()) {
			...
			try {
				...
				yearOfEmployment = Integer.parseInt(txfYearOfEmployment.getText().trim());
			} catch (NumberFormatException ex) {
				// do nothing
			}
			if (wage < 0) {
				lblError.setText("Wage is not a positive number");
			} else if (yearOfEmployment < 0) {
				lblError.setText("Invalid year");
			} else {
				...
				if (employee != null) {
					Controller.updateEmployee(employee, name, wage, employee.getYearOfEmployment());
