...
public class EmployeeWindow extends Stage {
	...
	private void initContent(GridPane pane) {
		...
		Label lblEmploy = new Label("Employment Year");
		pane.add(lblEmploy, 0, 4);
	...
	private void initControls() {
		if (employee != null) {
			...
			if (employee.getCompany() != null) {
				...
				txfEmploymentYear.setText(""+employee.getYearOfEmployment());
			} else {
	...
	private void okAction() {
		...
		if (name.length() == 0) {
			...
			try {
				...
				yearOfEmployment = Integer.parseInt(txfEmploymentYear.getText().trim());
			...
			if (wage < 0) {
				...
				lblError.setText("Year is not valid");
			}
			else {
